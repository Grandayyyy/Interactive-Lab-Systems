...
// docker run --name ILS_openvpn -v ${process.env.OPENVPN_VOLUME}:/etc/openvpn -d -p 1194:1194/udp --cap-add=NET_ADMIN kylemanna/openvpn
	...
	return new Promise(async (resolve, reject) => {
		...
		try {
			...
			containers.forEach((containerInfo) => {
				if(containerInfo.Names.includes("/ILS_openvpn")) {
