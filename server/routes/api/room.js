...
router.post("/launch", requiresLogin, async (req, res) => {
	...
	if(usage + price > user.tokens) {
		return res.json({
			success: false,
			error: "Please destroy some other boxes before launching this one!"
		});
	}
	...
	await utils.killUserContainer(user, `ILS_vpn_${username}_${name}`);
	...
	for (let i = 0; i < images.length; i++) {
		...
		let room_create = {
			Image: images[i],
			name: `ILS_room_${username}_${name}_${i}`,
			Tty: false,
			HostConfig: {
				NetworkMode: `container:ILS_vpn_${username}_${name}`,
				Memory: 512000000,
				CpuPeriod: 100000,
				CpuQuota: 100000,
				PidsLimit: 512,
		...
		await utils.killUserContainer(user, `ILS_room_${username}_${name}_${i}`);
